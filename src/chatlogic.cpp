...
ChatLogic::~ChatLogic()
{
    ...
    // delete _chatBot;
    /*
    std::cout<<"~ChatLogic  "<<"; _chatBot = "<<_chatBot<<"\n";
    if (_chatBot != nullptr){
        delete _chatBot;
        _chatBot = nullptr;
    }

    std::cout<<"~ChatLogic 2 "<<"; _chatBot = "<<_chatBot<<"\n";
    // delete all nodes
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        if (newNode == _nodes.end())
                        {
                            ...
                            _nodes.emplace_back(std::make_unique<GraphNode>(id)); // TASK3 - unique pointer
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            auto edge = std::make_unique<GraphEdge>(id); //replaced raw pointer with unique pointer
                            // edge->SetChildNode(*childNode);
                            edge->SetChildNode((*childNode).get());
                            // edge->SetParentNode(*parentNode);
                            edge->SetParentNode((*parentNode).get());
                            // _edges.push_back(edge);  // edges are moved to graph node
